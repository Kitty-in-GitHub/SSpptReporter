...
第三页内容。`,
    ...
    expect(result.action?.slide_action).toEqual({ goto: 3 });
    expect(result.action?.utterance).toBe("第三页内容。");
    expect(result.action?.action_id).toBe("p03");
  });
...
第二页内容。`,
    ...
    expect(result.action?.slide_action).toEqual({ next: true });
  });
});
