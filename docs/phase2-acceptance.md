...
| 5 | 打断 | 先 **播放本场讲稿**，播放中提问 | `barge_in` 停止当前句并答问；默认**不**续播讲稿；勾选「问答打断后自动续播讲稿」后可续播 |
| 6 | 语音输入 | Chrome / Edge：评委提问面板选语音引擎 → 麦克风 | 默认 Web Speech；可选本机/云端 Whisper（见 [`phase2-5-asr.md`](./phase2-5-asr.md)） |
| 7 | 演讲模式语音 | 进入 **演讲模式**，鼠标移到顶部唤出栏 | 可见 🎤 按钮；说中文后同样自动提问（与底部面板行为一致） |
| 8 | 请重复 | 提问「请重复一下」 | 直接复述上一轮回答，不调用 LLM |
| 9 | 场次切换 | 汇报工具栏 **场次** 下拉 | 可切换 demo / 私有 deck，PDF 与 FAQ 随之切换 |
